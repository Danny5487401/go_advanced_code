...
const (
	...
	defaultName = "world"
	sockP       = "/tmp/grpc_unix.sock"
)

func UnixConnect(context.Context, string) (net.Conn, error) {
	unixAddress, err := net.ResolveUnixAddr("unix", sockP)
...
func getDial(isTcp bool) (*grpc.ClientConn, error) {
	...
	return grpc.Dial(sockP, grpc.WithInsecure(), grpc.WithBlock(), grpc.WithContextDialer(UnixConnect))
}
